...
training_data <- NYA_rand[1:1000, c("latitude","longitude", "price")]
testing_data = NYA_rand[-(1:1000), c("latitude","longitude", "price")]
...
plot(longitude,latitude,col=NYA_rand$room_type,pch=1, main = "Neighbourhood groups")
#legend("topleft", legend=c("Brooklyn","Manhattan","Queens","Staten Island","Bronx") , col=c("Red","Green","Blue","Cyan","Black"))

#plot(longitude,latitude,col=Entire_home$price, pch=1, main = "Neighbourhood groups")

# Plot by Location with prices
#plot(longitude,latitude,col=heat.colors(Entire_home$price), pch=16, main = "Price")
